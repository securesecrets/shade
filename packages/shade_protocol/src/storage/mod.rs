...
pub trait NaiveBucketStorage: Serialize + DeserializeOwned {
    ...
    fn may_load<'a, S: Storage>(storage: &'a S, namespace: &'a [u8], key: &'a [u8]) -> StdResult<Option<Self>>{
    ...
    fn save<'a, S: Storage>(&self, storage: &'a mut S, namespace: &'a [u8], key: &'a [u8]) -> StdResult<()>{
