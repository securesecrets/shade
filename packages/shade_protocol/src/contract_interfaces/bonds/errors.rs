...
impl CodeType for Error {
    fn to_verbose(&self, context: &Vec<&str>) -> String {
        match self {
            Error::BondEnded => build_string("Bond ended on {}, it is currently {}", context),
            Error::BondNotStarted => build_string("Bond begins on {}, it is currently {}", context),
            Error::BondLimitReached => build_string(
                "Bond opportunity is not available due to issuance limit of {} being reached",
                context,
            ),
            ...
            Error::ContractNotActive => build_string(
                "Bonds contract is currently not active. Governance must activate the contract before functionality can resume.",
                context,
            ),
            Error::NoBondFound => {
                build_string("No bond opportunity found for deposit contract {}", context)
            }
            Error::NoPendingBonds => build_string("No pending bonds for user address {}", context),
            Error::BondLimitExceedsGlobalLimit => build_string(
                "Proposed bond issuance limit of {} exceeds available bond limit of {}",
                context,
            ),
            ...
            Error::BondIssuanceExceedsAllowance => build_string(
                "Bond issuance limit of {} exceeds available allowance of {}",
                context,
            ),
            Error::NotLimitAdmin => build_string(
                "Global limit parameters can only be changed by the limit admin",
                context,
            ),
            Error::DepositPriceExceedsLimit => build_string(
                "Deposit asset price of {} exceeds limit price of {}, cannot enter bond opportunity",
                context,
            ),
            Error::IssuedPriceBelowMinimum => build_string(
                "Issued asset price of {} is below minimum value of {}, cannot enter opportunity",
                context,
            ),
            Error::SlippageToleranceExceeded => build_string(
                "Calculated issuance amount of {} is below minimum accepted value of {}",
                context,
            ),
            Error::PermitContractMismatch => build_string("Permit isn't valid for {}", context),
            Error::PermitRevoked => build_string("Permit is revoked for user {}", context),
            Error::Blacklisted => build_string(
                "Cannot enter bond opportunity, sender address of {} is blacklisted",
                context,
            ),
            Error::NotTreasuryBond => build_string(
                "Cannot perform function since this is not a treasury bond",
                context,
            ),
            ...
            Error::QueryAuthBadResponse => build_string(
                "Query Authentication returned unrecognized response, cannot access information",
                context,
            ),
        }
    }
...
pub fn bond_not_started(start: u64, current: u64) -> StdError {
    DetailedError::from_code(BOND_TARGET, Error::BondNotStarted, vec![
        &start.to_string(),
        &current.to_string(),
    ])
    .to_error()
...
pub fn bond_ended(end: u64, current: u64) -> StdError {
    DetailedError::from_code(BOND_TARGET, Error::BondEnded, vec![
        &end.to_string(),
        &current.to_string(),
    ])
    .to_error()
...
pub fn mint_exceeds_limit(mint_amount: Uint128, available: Uint128) -> StdError {
    ...
    DetailedError::from_code(BOND_TARGET, Error::MintExceedsLimit, vec![
        mint_str,
        available_str,
    ])
    .to_error()
...
pub fn no_bond_found(deposit_asset_address: &str) -> StdError {
    DetailedError::from_code(BOND_TARGET, Error::NoBondFound, vec![deposit_asset_address])
        .to_error()
}
...
pub fn bond_limit_exceeds_global_limit(
    ...
    DetailedError::from_code(BOND_TARGET, Error::BondLimitExceedsGlobalLimit, vec![
        bond_limit_str,
        available_str,
    ])
    .to_error()
...
pub fn bonding_period_below_minimum_time(
    ...
    DetailedError::from_code(BOND_TARGET, Error::BondingPeriodBelowMinimumTime, vec![
        bond_period_str,
        global_minimum_bonding_period_str,
    ])
    .to_error()
}
...
pub fn bond_discount_above_maximum_rate(
    ...
    DetailedError::from_code(BOND_TARGET, Error::BondDiscountAboveMaximumRate, vec![
        bond_discount_str,
        global_maximum_discount_str,
    ])
    .to_error()
}
...
pub fn bond_issuance_exceeds_allowance(
    ...
    let bond_limit_str = bond_limit_string.as_str();
    DetailedError::from_code(BOND_TARGET, Error::BondIssuanceExceedsAllowance, vec![
        bond_limit_str,
        available_str,
    ])
    .to_error()
...
pub fn deposit_price_exceeds_limit(deposit_price: Uint128, limit: Uint128) -> StdError {
    ...
    DetailedError::from_code(BOND_TARGET, Error::DepositPriceExceedsLimit, vec![
        deposit_str,
        limit_str,
    ])
    .to_error()
...
pub fn issued_price_below_minimum(issued_price: Uint128, limit: Uint128) -> StdError {
    ...
    let limit_string = limit.to_string();
    let limit_str = limit_string.as_str();
    DetailedError::from_code(BOND_TARGET, Error::IssuedPriceBelowMinimum, vec![
        issued_str, limit_str,
    ])
    .to_error()
}
...
pub fn slippage_tolerance_exceeded(
    ...
    DetailedError::from_code(BOND_TARGET, Error::SlippageToleranceExceeded, vec![
        issue_str,
        min_amount_str,
    ])
    .to_error()
...
pub fn permit_contract_mismatch(expected: &str) -> StdError {
    DetailedError::from_code(BOND_TARGET, Error::PermitContractMismatch, vec![expected]).to_error()
}
