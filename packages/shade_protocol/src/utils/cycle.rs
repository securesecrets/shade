use cosmwasm_std::{
    Uint128, StdResult, StdError, Env,
};
use crate::{
    utils::{
        asset::Contract, 
    ...
    DateTime::parse_from_rfc3339(&rfc3339)
        ...
        .map_err(|_| 
            StdError::generic_err(
                format!("Failed to parse rfc3339 datetime {}", rfc3339)
            )
        )
}
...
pub fn utc_now(
    ...
    DateTime::from_utc(
        NaiveDateTime::from_timestamp(env.block.time as i64, 0), 
        Utc,
    )
}

pub fn exceeds_cycle(
    now: &DateTime<Utc>,
    ...
    match cycle {
        ...
        Cycle::Minutes { minutes } => {
            minutes >= Uint128(((now.timestamp() - last_refresh.timestamp()) / 60).try_into().unwrap())
        },
        Cycle::Hourly { hours } => {
            hours >= Uint128(((now.timestamp() - last_refresh.timestamp()) / 60 / 60).try_into().unwrap())
        },
        Cycle::Daily { days } => {
        ...
        Cycle::Yearly { years } => {
            now.year_ce() > last_refresh.year_ce()
        },
    }
