...
macro_rules! implement_harness_lp {
    ($x:ident, $s:ident) => {
        use cosmwasm_std::{from_binary, Binary, Env, HandleResponse, InitResponse, StdResult};
        use fadroma_ensemble::{ContractHarness, MockDeps};
        impl ContractHarness for $x {
            fn init(&self, deps: &mut MockDeps, env: Env, msg: Binary) -> StdResult<InitResponse> {
                $s::init(deps, env, from_binary(&msg)?)
            }
            ...
            fn handle(
                ...
                $s::handle(deps, env, from_binary(&msg)?)
            }

            fn query(&self, deps: &MockDeps, msg: Binary) -> StdResult<Binary> {
                $s::query(deps, from_binary(&msg)?)
            }
        }
    };
}

pub(crate) use implement_harness_lp;
