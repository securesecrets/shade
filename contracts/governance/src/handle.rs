...
pub fn try_register_asset<S: Storage, A: Api, Q: Querier>(
    ...
    */
    ...
    match assets.may_load(contract.address.to_string().as_bytes())? {
        Some(_) => {}, //return Err(StdError::generic_err("Asset already exists")),
        None => {
            assets.save(contract.address.to_string().as_bytes(), &Snip20Asset {
            ...
            asset_list(&mut deps.storage).update(|mut state| {
                state.push(contract.address.to_string());
                Ok(state)
            })?;
            */
