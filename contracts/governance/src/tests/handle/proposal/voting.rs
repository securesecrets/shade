...
use crate::tests::{get_proposals, Governance, init_governance, Snip20, StkdShd};

fn init_voting_governance_with_proposal(
) -> StdResult<(ContractEnsemble, ContractLink<HumanAddr>, ContractLink<HumanAddr>)>
{
    ...
    let stkd_tkn = chain.register(Box::new(StkdShd));
    ...
    // Stake tokens
    chain.execute(&snip20_reference_impl::msg::HandleMsg::Send {
        recipient: stkd_tkn.address.clone(),
        recipient_code_hash: None,
        amount: cosmwasm_std::Uint128(20_000_000),
        memo: None,
        msg: Some(to_binary(&shd_staking::ReceiveType::Bond { useFrom: None }).unwrap()),
        padding: None
    }, MockEnv::new(
            "alpha",
            ContractLink {
        ...
        &InitMsg {
            ...
            admin_profile: Profile {
                ...
                token: Some(VoteProfile {
                    deadline: 10000,
                    threshold: Count::LiteralCount { count: Uint128::new(10_000_000) },
                    yes_threshold: Count::LiteralCount { count: Uint128::new(15_000_000) },
                    veto_threshold: Count::LiteralCount { count: Uint128::new(15_000_000) }
                }),
                cancel_deadline: 0
            },
...
#[test]
fn invalid_vote() {
    let (mut chain, gov, stkdTkn) = init_voting_governance_with_proposal().unwrap();

    assert!(
        chain.execute(
            &shd_staking::HandleMsg::ExposeBalance {
                recipient: gov.address,
                code_hash: None,
                msg: Some(to_binary(&governance::vote::ReceiveBalanceMsg {
                    vote: Vote {
            ...
            MockEnv::new(
                "alpha",
                ContractLink {
                    address: stkdTkn.address.clone(),
                    code_hash: stkdTkn.code_hash.clone(),
                }
            )
        ).is_err()
    );
...
fn vote_yes() {
    ...
    assert_eq!(prop.public_vote_tally, Some(Vote {
        yes: Uint128::new(1_000_000),
        no: Uint128::zero(),
