...
pub fn try_transfer_impl<S: Storage>(
    ...
    if !Config::transfer_enabled(&deps.storage)? {
    ...
    if owner.is_some() {
        Allowance::spend(&mut deps.storage, some_owner, sender, amount, block)?;
    }

    Balance::transfer(&mut deps.storage, amount, some_owner, recipient)?;
...
pub fn try_transfer<S: Storage, A: Api, Q: Querier>(
    ...
    try_transfer_impl(deps, &env.message.sender, None, &recipient, amount, memo, denom, &env.block)?;
    Ok(HandleResponse{
        messages: vec![],
        log: vec![],
...
pub fn try_batch_transfer<S: Storage, A: Api, Q: Querier>(
    ...
    for action in actions {
        try_transfer_impl(deps, &sender, None, &action.recipient, action.amount, action.memo, denom.clone(), &block)?;
    }
    Ok(HandleResponse{
        messages: vec![],
        log: vec![],
...
fn try_add_receiver_api_callback<S: Storage>(
    ...
    let receiver_hash = match recipient_code_hash {
        None => ReceiverHash::may_load(&storage, recipient.clone())?.0,
        Some(hash) => Some(hash)
    };
    ...
    if let Some(hash) = receiver_hash {
        messages.push(
            ReceiverHandleMsg::new(sender, from, amount, memo, msg)
                .to_cosmos_msg(hash, recipient, None)?
        );
    }
    Ok(())
}
...
pub fn try_send_impl<S: Storage>(
    ...
    try_transfer_impl(deps, &env.message.sender, owner, &recipient, amount, memo.clone(), denom, &env.block)?;
