...
use contract_harness::harness::snip20_t::Snip20T;
use fadroma_platform_scrt::ContractLink;
use shade_protocol::contract_interfaces::snip20_test;
use shade_protocol::contract_interfaces::snip20_test::{InitConfig, InitialBalance};

//TODO: test rng

pub fn init_snip20(
    msg: snip20_test::InitMsg
) -> StdResult<(ContractEnsemble, ContractLink<HumanAddr>)> {
    let mut chain = ContractEnsemble::new(50);

    // Register governance
    let gov = chain.register(Box::new(Snip20T));
...
pub fn init_snip20_with_config(
    ...
    let (mut chain, snip) = init_snip20(snip20_test::InitMsg {
...
pub fn create_vk(
    ...
    chain.execute(&snip20_test::HandleMsg::SetViewingKey {
