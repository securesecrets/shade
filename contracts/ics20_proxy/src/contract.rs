...
use shade_protocol::{
    c_std::{
        entry_point,
        from_binary, to_binary, Addr, Binary, Deps, DepsMut, Env, IbcMsg, IbcQuery, MessageInfo,
        PortIdResponse, StdError, StdResult,
        Uint128, Response,
    },
};
...
const CONTRACT_NAME: &str = "crates.io:snip20-ics20";
...
pub fn instantiate(
    ...
    NATIVE_TOKEN.save(deps.storage, &msg.native_token)?;
...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::Receive { sender, from, amount, memo, msg } => execute_receive(deps, env, info, sender, from, amount, memo, msg),
        /*
...
pub fn execute_receive(
    ...
    //increase_channel_balance(deps.storage, &msg.channel, &"denom".into(), amount)?;
...
pub fn execute_transfer(
    ...
    if !CHANNEL_INFO.has(deps.storage, &msg.channel) {
        return Err(StdError::generic_err("No Channel"));
    }
    */
    let config = CONFIG.load(deps.storage)?;
...
mod test {
    ...
    fn proper_checks_on_execute_cw20() {
        ...
        let msg = ExecuteMsg::Receive(Snip20ReceiveMsg {
