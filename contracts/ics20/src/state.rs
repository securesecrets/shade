...
pub const NATIVE_TOKEN: Item<Contract> = Item::new("native_token");
...
pub fn reduce_channel_balance(
    ...
    CHANNEL_STATE.update(
        ...
        |orig| -> Result<_, ContractError> {
            ...
            let mut cur = orig.ok_or(StdError::generic_err("Insufficient Funds"))?:
            cur.outstanding = cur
                .outstanding
                .checked_sub(amount)
                .or(Err(StdError::generic_err("Insufficient Funds")))?;
            Ok(cur)
        },
    )?;
    Ok(())
