...
fn enforce_order_and_version(
    ...
    if channel.version != ICS20_VERSION {
        return Err(StdError::generic_err("Invalid IBC version"));
    }
    ...
    if channel.order != ICS20_ORDERING {
        return Err(StdError::generic_err("Only ordered channel"));
    }
    Ok(())
}
...
fn parse_voucher_denom<'a>(
    ...
    if split_denom.len() != 3 {
        return Err(StdError::generic_err("No foreign tokens"));
    }
...
fn on_packet_success(_deps: DepsMut, packet: IbcPacket) -> StdResult<IbcBasicResponse> {
    let msg: Ics20Packet = from_binary(&packet.data)?;
...
mod test {
    ...
    use cosmwasm_std::{coins, to_vec, IbcEndpoint, Ics20Packet, IbcTimeout, Timestamp};
    ...
    fn send_receive_cw20() {
        ...
        assert_eq!(
            &res.messages[0],
            &SubMsg::new(Ics20Packet::SendPacket {
