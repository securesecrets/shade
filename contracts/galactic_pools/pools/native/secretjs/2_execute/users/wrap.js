...
async function wrap (amt = 10) {
  ...
  const contractAddress = process.env.SSCRT_ADDRESS
  const codeHash = process.env.SSCRT_HASH
  ...
  let sim = await secretjs.tx.compute.executeContract.simulate({
    ...
    sentFunds: [{ amount, denom: 'uscrt' }] // optional
  })
  ...
  console.log(`Wrapped ${amount / process.env.SCRT_TO_USCRT} Scrt successfully`)
}
export default wrap
