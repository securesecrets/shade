...
use shade_protocol::{
    c_std::{Decimal, Deps, Env, Fraction, Uint128},
    utils::Query,
};
...
pub fn ctoken_info(deps: Deps, config: &Config) -> Result<TokenInfoResponse, ContractError> {
    lend_token::msg::QueryMsg::TokenInfo {}.query(&deps.querier, &config.ctoken_contract)
}

pub fn query_ctoken_multiplier(deps: Deps, cfg: &Config) -> Result<Decimal, ContractError> {
    let resp: lend_token::msg::MultiplierResponse =
        lend_token::QueryMsg::Multiplier {}.query(&deps.querier, &cfg.ctoken_contract)?;
    Ok(resp.multiplier)
}
