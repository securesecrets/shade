...
use utils::{amount::token_to_base, price::PriceRate};
use wyndex::{
...
impl SuiteBuilder {
    ...
    pub fn with_contract_funds(mut self, funds: utils::coin::Coin) -> Self {
    ...
    pub fn with_agency_funds(mut self, funds: utils::coin::Coin) -> Self {
        let funds = Coin::try_from(funds).unwrap();
...
impl Suite {
    ...
    pub fn create_pool_and_provide_liquidity(
        &mut self,
        (coin1, coin2): (utils::coin::Coin, utils::coin::Coin),
        pair_type: PairType,
    ) -> AnyResult<Addr> {
        let owner = self.owner.clone();
        let oracle = self.oracle_contract.clone();
    ...
    pub fn create_pool_with_liquidity_and_twap_price(
        &mut self,
        coin1: utils::coin::Coin,
        coin2: utils::coin::Coin,
        pair_type: PairType,
    ) -> Addr {
        let pair = self
    ...
    pub fn repay(&mut self, sender: &str, funds: utils::coin::Coin) -> AnyResult<AppResponse> {
        use Token::*;
        match funds.denom {
    ...
    pub fn query_withdrawable(&self, account: impl ToString) -> AnyResult<utils::coin::Coin> {
        let response: utils::coin::Coin = self.app.wrap().query_wasm_smart(
            self.contract.clone(),
    ...
    pub fn query_borrowable(&self, account: impl ToString) -> AnyResult<utils::coin::Coin> {
        let response: utils::coin::Coin = self.app.wrap().query_wasm_smart(
            self.contract.clone(),
            &QueryMsg::Borrowable {
                account: account.to_string(),
