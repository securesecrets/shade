...
fn require_single_denom(funds: &[StdCoin]) -> Result<lending_utils::coin::Coin, ContractError> {
    if funds.len() != 1 {
        return Err(ContractError::RequiresExactlyOneCoin {});
    };
    Ok(lending_utils::coin::Coin {
...
mod tests {
    ...
    fn divide_u128_by_decimal_rounding() {
        assert_eq!(
            cr_lending_utils::divide(60u128.into(), Decimal::percent(60)).unwrap(),
            Uint128::new(100u128)
        );
    }
}
