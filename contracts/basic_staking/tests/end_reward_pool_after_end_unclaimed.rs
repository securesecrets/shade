...
fn end_reward_pool_after_end() {
    ...
    let end_period = (reward_end + Uint128::new(100));
    ...
    {
        Ok(_) => panic!("Reward pool cancelled before fully claimed"),
        Err(_) => {}
    }
    ...
    {
        basic_staking::QueryAnswer::RewardPools { rewards } => {
            assert_eq!(rewards.len(), 1, "Unclaimed reward pool was not removed");
        }
        _ => {
            panic!("Staking balance query failed");
        }
    ...
    .unwrap()
    {
        snip20::QueryAnswer::Balance { amount } => {
            assert_eq!(
                amount,
                Uint128::zero(),
                "No pending emissions when cancelled",
            );
        }
        _ => {
