...
#[test]
fn end_reward_pool_before_end_unclaimed() {
    ...
    let mid_period = reward_start + ((reward_end - reward_start) / Uint128::new(2));
    println!("mid period {}", mid_period);

    // move to middle of emissions period
    app.set_block(BlockInfo {
        height: 1,
    ...
    {
        basic_staking::QueryAnswer::RewardPools { rewards } => {
            assert_eq!(rewards.len(), 1, "Reward pool remains until fully claimed");
            assert_eq!(
                rewards[0].amount,
                reward_amount / Uint128::new(2),
                "Reward pool half remaining",
            );
        }
        _ => {
            panic!("Staking balance query failed");
    ...
    .unwrap()
    {
        snip20::QueryAnswer::Balance { amount } => {
            assert_eq!(
                amount,
                reward_amount / Uint128::new(2),
                "Cancelled rewards sent to cancelling admin user"
            );
        }
        _ => {
